...
<!-- for header  -->            
  ...
  <table class="table">
      ...
      <tr>
        ...
        <td colspan="1"><center><h3><b>HARGA TOTAL</b></h3></center></td>
      </tr>
       <tr v-for="item of tambahan">
        ...
        <td colspan="1"><center><h3><b>{{formatNumberRupiah(formatNumber(total(tambahan)))}}</b></h3></center></td>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click.prevent="ApproveData()">Finish</button>
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectProcurementApproval"
      :fields="fields"
      pagination-path=""
...
fetchIt(){
                axios.get('/karyawan/GetProfileProcurementApproval?token=' + this.token).then((response) => {
...
var masuk ={projectid:this.PesanData.id,tenantid:this.PesanData.tenantscode,message:this.message};
              ...
              axios.post('/karyawan/ProcurementApproval1', masuk)
                    .then(response => {
if(response.data.success)
{
                  this.errorNya = '';
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetProjectProcurementApproval').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
