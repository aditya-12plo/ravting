...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4>Tambah Data Akses Tenant</h4></template>
        <template slot="body" >
            <form method="POST" action="" @submit.prevent="submitBooks()">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control border-input" placeholder="Tenant ID" v-model="forms.tenantid"  type="text" required>
                        <span class="label label-danger" v-for="error of errorNya['tenantid']">
                        {{ error }}
                    </span>
                    </div>
...
<!-- @view --->
        ...
        <template slot="header" ><h4>View Tenant</h4></template>
        <template slot="body" >
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="tenantid">Tenant ID :  {{forms.tenantid}}</label>
...
<!-- @update --->
        ...
        <template slot="header" ><h4>Edit Data Tenant</h4></template>
        <template slot="body" >
 ...
 <!-- @delete --->
        ...
        <template slot="header" ><h4>Delete Tenant</h4></template>
        <template slot="body" >
...
<!-- @upload Modal--->
        ...
        <template slot="header" ><h4>Upload Penambahan Data Tenant Baru</h4></template>
        <template slot="body" >
            <form method="POST" action="" enctype="multipart/form-data" @submit.prevent="uploadBooks()">
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="name">Download Template Tenant : <a href="/template/uploaddatatenant.xlsx" target="_blank">Download</a></label>
                    </div>
...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <tr>
      ...
      <td colspan="3"><input type="text" v-model="filterText" class="form-control" @keyup.enter="doFilter" placeholder="Nama / Tenant ID"></td>
    </tr>
     <tr>
...
export default {
  ...
  data () {
	...
	file_name:'',
    errors: new Errors() ,
   forms:new CrudForm({index:'',  id:'' , name:'' ,  tenantid:'', created_at:''}) ,
  ...
  let masuk = new FormData();
   masuk.set('file_name', this.file_name)
              axios.post('/karyawan/uploadfiletenant' , masuk)
                    .then(response => {
...
{				 this.modal.set('upload', false);
  ...
  symbol: " M",precision: 0,format: "%v %s"})
    ...
    submitBooks() {
	 axios.post('/karyawan/AddTenant', this.forms)
                    .then((response) => {
                this.errorNya = '';
                this.resetFilter();
   ...
   updateItem() {
axios.put('/karyawan/UpdateTenant/'+ this.forms.id , this.forms)
                    .then(response => {
                this.errorNya = '';
                 this.submitted = this.showTime('success',response.data.success);
                this.resetFilter();
            ...
            destroyItem(kode){
                axios.delete('/karyawan/DeleteTenant/'+ kode)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('karyawan/listtenant').then((response) => {
                    this.dataNya = response.data;
                    this.loading = false
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
