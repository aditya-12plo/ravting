...
<!-- @view --->
        ...
        <template slot="body" >
        <form method="POST" action="" @submit.prevent="ApproveData()" enctype="multipart/form-data">
...
<!-- for header  -->            
  ...
  <table class="table">
      ...
      <tr v-if="PesanData.status_id > 0">
        <td><h5 align="left"><b>Pesan Terakhir</b></h5></td>
      ...
      <tr>
       ...
       <tr v-for="(row,index) of this.tambahan">
        <td><div v-if="row.status == 'additional'"><input type="text" v-model="row.workitem" class="form-control" required></div><div v-else>{{row.workitem}}</div></td>
      <td><div v-if="row.status == 'additional'"><input type="text" v-model="row.unit"  maxlength="10" class="form-control" required></div><div v-else>{{row.unit}}</div></td>
      <td><div v-if="row.status == 'additional'"><input v-on:keypress="isNumber()"  maxlength="4" v-model="row.volume" class="form-control" required></div><div v-else>{{row.volume}}</div></td>
      <td><div v-if="row.status == 'additional'"><input v-on:keypress="isNumber()"  maxlength="13" v-model="row.unitprice" class="form-control" required></div><div v-else>{{row.unitprice}}</div></td>
      <td>{{formatNumberRupiah(formatNumber(row.volume * row.unitprice))}}</td>
      <td><div v-if="row.status == 'additional'"><button class="button btn-danger" @click="removeRow(index)">Hapus</button></div><div v-else>standart</div></td>
       
      </tr>
         <tr>
        <td colspan="4"><center><h3><b>Grand Total Biaya Pekerjaan</b></h3></center></td>
        <td colspan="1"><center><h3><b>{{formatNumberRupiah(formatNumber(total(this.tambahan)))}}</b></h3></center></td>
      </tr>
  </table>
</div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button"  @click.prevent="BODItem()" class="btn btn-warning">BOD Approval</button>
                     <button type="button" class="btn btn-danger" @click.prevent="RejectItem()">Reject</button>
                </div>
                </form>
        </template>
        </modal>
...
<!-- @bod --->
        <modal  v-if="modal.get('bod')" @close="modal.set('bod', false)">
        <template slot="header" ><h4>Alasan Project Dikirim Ke BOD</h4></template>
        <template slot="body" >

            <form method="POST" action="" @submit.prevent="BODupdateItem()">
                <div class="modal-body">
...
<span class="label label-danger" v-for="error of errorNya['message']">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" @click="modal.set('bod', false)" >Close</button>
                    <button type="submit" class="btn btn-primary">Kirim</button>
                </div>
            </form>
        </template>
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectUbisApproval2"
      :fields="fields"
      pagination-path=""
...
export default {
  ...
  option: {
    ...
    file_name:'',
	pesannya:[],
    modal:new CrudModal({view:false, reject:false, bod:false, approve:false, message:false}),
     berhasilNya: [],
     errorNya: [],
     workitem:[],
     url:'/files/',
  ...
  addRow: function(){
     this.tambahan.push({workitem:"",unit:"",volume:"",unitprice:"",status:"additional"});
    },
...
fetchIt(){
                axios.get('/karyawan/GetProfileUbisEGM?token=' + this.token).then((response) => {
...
{	
   ...
   BODupdateItem() {
 var masuk ={projectid:this.PesanData.id,tenantid:this.PesanData.tenantscode,implementation:this.tambahan,message:this.message};

              axios.post('/karyawan/BODupdateItem', masuk)
                    .then(response => {
if(response.data.success)
{	
                  this.errorNya = '';
  ...
  ApproveData(){
   ...
   var masuk ={projectid:this.PesanData.id,tenantid:this.PesanData.tenantscode,implementation:this.tambahan,message:this.message};
              ...
              axios.post('/karyawan/UbisApproval2', masuk)
                    .then(response => {
if(response.data.success)
{
                  this.errorNya = '';
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetProjectUbisApproval2').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
