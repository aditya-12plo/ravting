...
<!-- for header  -->            
  ...
  <table class="table">
      ...
      <tr>
        ...
        <td colspan="4"><h5 align="left"><b>{{PesanData.header.projectid}}</b></h5></td>
      </tr>  
	  <tr>
        <td><h5 align="left"><b>Tenant</b></h5></td>
        <td colspan="4"><h5 align="left"><b>{{PesanData.header.name}}</b></h5></td>
      </tr>
      <tr>
        <td><h5 align="left"><b>Nama Lokasi</b></h5></td>
        <td colspan="4"><h5 align="left"><b>{{PesanData.header.sitename}}</b></h5></td>
      </tr>
      <tr>
        <td><h5 align="left"><b>Daerah</b></h5></td>
        <td colspan="4"><h5 align="left"><b>{{PesanData.header.regional}}</b></h5></td>
      </tr>
      <tr>
        <td><h5 align="left"><b>Nilai Project</b></h5></td>
        <td colspan="4"><h5 align="left"><b>{{formatNumberRupiah(PesanData.header.projectprice)}}</b></h5></td>
      </tr>
      <tr>
        <td><h5 align="left"><b>Tinggi Tower</b></h5></td>
        <td colspan="4"><h5 align="left"><b>{{PesanData.header.tower}} M</b></h5></td>
      </tr>
      <tr>
        <td><h5 align="left"><b>File Upload</b></h5></td>
        <td colspan="4"><div class="form-group"><div v-if="PesanData.header.filename"><a v-bind:href="url+PesanData.header.filename" target="_blank"> download </a></div>
                    </div></td>
      </tr>
	  <tr v-if="PesanData.header.status_id > 0">
        <td><h5 align="left"><b>Komunikasi Project</b></h5></td>
        <td colspan="4"><button type="submit" class="btn btn-default" @click.prevent="CheckMessage()">Lihat</button></td>
      </tr>
      <tr>
      ...
      <tr>
       ...
       <tr v-for="item of PesanData.standart">
        ...
        <td>{{formatNumberRupiah(formatNumber(item.total))}}</td>
         ...
         <tr>
        <td colspan="4"><center><b>Total Biaya Pekerjaan Standar</b></center></td>
        <td><b>{{formatNumberRupiah(formatNumber(total(PesanData.standart)))}}</b></td>
      </tr>
	<tr>
        <td colspan="5"> <div style="height: 50px; overflow:hidden;"></div></td>
      </tr>
      ...
      <tr>
       ...
       <tr v-for="item of PesanData.additional">
        ...
        <td><b>{{formatNumberRupiah(formatNumber(total(PesanData.additional)))}}</b></td>
      </tr>
      ...
      <tr>
       ...
       <tr v-for="item of PesanData.reduction">
        ...
        <td><b>{{formatNumberRupiah(formatNumber(total(PesanData.reduction)))}}</b></td>
      </tr>
	  ...
	  <tr>
        ...
        <td><b>{{formatNumberRupiah(formatNumber((total(PesanData.standart)+total(PesanData.additional))- total(PesanData.reduction)))}}</b></td>
      </tr>
...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectFinal"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
export default {
  ...
  resetOptions() {
       ...
       axios.get('/karyawan/GetProjectDetailProses/'+ item.id).then((response) => {
			...
			CheckMessage() {
			axios.get('/karyawan/GetMessageNya/'+this.PesanData.header.id).then((response) => {
  ...
  Object.entries(data).forEach(([key, val]) => {
      total.push(val.total) // the value of the current key.
  });
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('karyawan/GetProjectFinal').then((response) => {
