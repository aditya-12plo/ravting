...
<!-- for header  -->            
  ...
  <table class="table">
      ...
      <tr>
       ...
       <tr v-for="(row,index) of tambahan">
      ...
      <td><div v-if="row.status == 'additional'"><button class="button btn-danger" @click="removeRow(index)">Hapus</button></div><div v-else>standart</div></td>
        ...
        <td colspan="1"><center><h3><b>{{formatNumberRupiah(formatNumber(total(tambahan)))}}</b></h3></center></td>
      </tr>
  </table>
</div>
                    </div>
...
<!-- @approve --->
        ...
        <template slot="header" ><h4>Kirim Pesan Ke Divisi PROCUREMENT</h4></template>
        <template slot="body" >
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectBODApproval"
      :fields="fields"
      pagination-path=""
...
fetchIt(){
                axios.get('/karyawan/GetProfileUbisBOD?token=' + this.token).then((response) => {
...
{	
  ...
  ApproveData(){
   ...
   var masuk ={projectid:this.PesanData.id,tenantid:this.PesanData.tenantscode,implementation:this.tambahan,message:this.message};
              ...
              axios.post('/karyawan/BODApproval', masuk)
                    .then(response => {
if(response.data.success)
{
                  this.errorNya = '';
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetProjectBODApproval').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
