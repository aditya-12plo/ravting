...
<div class="col-sm-12">
 ...
 <div class="col-sm-6">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="tower">Tinggi Tower : {{this.forms.tower}}</label>
                    </div>
 ...
 <table class="table">
     ...
     <tr>
      ...
      <tr>
        ...
        <td align="right"><money v-model="forms.lahan" class="form-control border-input" placeholder="Sewa Lahan (11 tahun include PPh)" v-bind="money"></money>                          
 ...
 <table class="table">
     ...
     <tr>
      ...
      <tr>
        ...
        <td align="center">{{this.forms.qty_lahan}}</td>
        <td align="right"><money v-model="forms.lahan" class="form-control border-input" placeholder="Sewa Lahan (Include PPh)" v-bind="uang"></money>                          
<span class="label label-danger" v-for="error of errorNya['lahan']">
                        {{ error }}
                    </span></td>
        <td align="right">{{formatNumberRupiah(this.forms.qty_lahan*this.forms.lahan)}}</td>
...
<div class="col-sm-12">             
<div class="small">
    <AreaDanPM :height="250" :data="this.simulasiNyaKedua"></AreaDanPM> 
  </div>
</div>  
<div class="col-sm-12">             
...
<div style="overflow-x:auto;">
  <table>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetRepairProjectAreaM"
      :fields="fields"
      pagination-path=""
...
import AreaDanPM from './AreaDanPM'
...
export default {
  ...
  option: {
    ...
    selectedtower: '',
    pesannya:[],
   forms:new CrudForm({index:'',  id:'' , no_doc:'' ,projectimplementationid:'' ,filename:'' ,  tinggitower:'' , proc:'' ,  ubis:'' , projectid:'' , sitename:'', siteid:'' , area:'', regional:'', cluster:'', kode_tower_depan:'' ,  tower:''  ,  kode_tower_belakang:'' , longitude:'' , latituted:'' , towerprovider:''  , infratype:'' , budget:'' ,  sis:'' ,  sitac:'' ,  imb:'' ,  imb_jasa:'' , qty_lahan:'', lahan:'' ,  biayatower:'' ,  cme:'' ,   pln:'' ,   addtransport:'' ,  addpln:'' ,  addcme:'' , provinsi:''  , kabupaten:''  , mitra:'' , created_at:''}) ,
...
this.isLoading = false;
  ...
  this.isLoading = false;
   ...
   ApproveItem() {
   this.isLoading = true;
 var masuk ={id:this.forms.id,projectimplementationid:this.forms.projectimplementationid,infratype:this.forms.infratype,projectid:this.forms.projectid,sitename:this.forms.sitename,kabupaten:this.forms.kabupaten,longitude:this.forms.longitude,latituted:this.forms.latituted,lahan:this.forms.lahan,qty_lahan:this.forms.qty_lahan,cme:this.forms.cme,pln:this.forms.pln,addtransport:this.forms.addtransport,addpln:this.forms.addpln,addcme:this.forms.addcme,biayatower:this.forms.biayatower,towerimplementation:this.forms.tower,clusterimplementation:this.forms.cluster,mitra:this.forms.mitra,standard:this.dataStandard,addtional:this.tambahan,message:this.message,area:this.forms.area,levelnya:'GENERAL MANAGER AREA',statusnya:'AREA - GENERAL MANAGER AREA'};
   axios.post('/karyawan/SubmitProjectRepair', masuk)
                    .then(response => {
...
onLoading() {
    ...
    onLoadingError() {
                this.isLoading = true;
                axios.get('/karyawan/GetRepairProjectAreaM').then((response) => {
                    this.isLoading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
