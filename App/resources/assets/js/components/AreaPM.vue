      ...
      <tr>
        ...
        <td align="right" colspan="2"><h4><b>{{formatNumberRupiah(TotalCapexB2S())}}</b></h4></td>
               ...
               <button type="button" class="btn btn-info" @click="SimulasiData()" >Simulasi Capex</button>
                    ...
                    <button type="button" class="btn btn-danger" @click="modal.set('reject', true)">Perbaiki Ke Manager Area</button>
...
<div style="overflow-x:auto;">
  <table>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectAreaPM"
      :fields="fields"
      pagination-path=""
...
fetchIt(){
this.isLoading = true;
                axios.get('/karyawan/GetProfileAreaPM').then((response) => {
...
var ttl = this.forms.sis+this.forms.sitac+this.forms.imb+this.forms.imb_jasa+(this.forms.qty_lahan*this.forms.lahan)+(this.dataStandard.tonase*this.dataStandard.biayatower)+this.dataStandard.erection_tower+this.dataStandard.transportasi+(this.dataStandard.pondasi_greenfiled*this.dataStandard.pondasi)+this.dataStandard.bts_outdoor+this.dataStandard.me+this.dataStandard.fy+this.dataStandard.pln_connect+this.total(this.tambahan);
   ...
   if(this.forms.towerimplementation < 42)
   {
		  if(ttl <= this.forms.budget)
   {
	var statusnya2 = 'PROCUREMENT - STAFF PROCUREMENT';
	var levelnya2 = 'STAFF PROCUREMENT';
   }
   else
   {
	var statusnya2 = 'UBIS - MANAGER';
	var levelnya2 = 'MANAGER UBIS';
   }
   }
   else
   {
 ...
 var masuk ={id:this.forms.id,projectimplementationid:this.forms.projectimplementationid,infratype:this.forms.infratype,projectimplementationid:this.forms.projectimplementationid,projectid:this.forms.projectid,sitename:this.forms.sitename,kabupaten:this.forms.kabupaten,qty_lahan:this.forms.qty_lahan,towerimplementation:this.forms.tower,clusterimplementation:this.forms.cluster,mitra:this.forms.mitra,lahan:this.forms.lahan,cme:this.forms.cme,pln:this.forms.pln,addtransport:this.forms.addtransport,addpln:this.forms.addpln,addcme:this.forms.addcme,biayatower:this.forms.biayatower,message:this.message,created:this.forms.created_at,standard:this.dataStandard,addtional:this.tambahan,levelnya:levelnya2,statusnya:statusnya2};

   axios.post('/karyawan/ApprovalProjectPM', masuk)
                    .then(response => {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            SubmitData(){
			  ...
			  RepairItem() {
var masuk ={id:this.forms.id,message:this.message,level:'MANAGER AREA',statusnya:'REPAIR AREA - MANAGER AREA'};
...
onLoading() {
    ...
    onLoadingError() {
                this.isLoading = true;
                axios.get('/karyawan/GetProjectAreaPM').then((response) => {
                    this.isLoading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
