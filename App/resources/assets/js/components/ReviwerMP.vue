      ...
      <tr v-if="this.levelnya==='DIRECTOR'">
        <td align="left" colspan="8"><h4><b>NOTES : * Project ini sudah mendapat Approval BOD Terkait</b></h4></td>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" @click.prevent="print()">Print</button>
...
fetchIt(){
this.isLoading = true;
                axios.get('/karyawan/GetProfileProcurementReviewerM').then((response) => {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            SubmitData(){
			  ...
			  RepairItem() {
var masuk ={id:this.forms.id,message:this.message,level:this.levelnya,statusnya:'REPAIR PROCUREMENT - STAFF PROCUREMENT'};
...
if(response.data.success)
{
 ...
 axios.get('/karyawan/GetProjectDetailProses/'+item.id+'/'+item.projectimplementationid+'/'+item.regional+'/'+item.area).then((response) => {
				...
				this.simulasiNya = response.data.simulasi;
