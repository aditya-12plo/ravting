...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4>Add New Department</h4></template>
        <template slot="body" >
            ...
            <div v-show="loading2"><i class="fa fa-spinner fa-spin"></i> Loading Company List</div>
<v-select v-model="forms.company_id" :options="pilihannya" label="company_name"></v-select>
			
                        <span class="label label-danger" v-for="error of errorNya['company_id']">
                        {{ error }}
                    </span> 
          <span class="label label-danger" v-for="error of errorNya['company_id']">
                        {{ error }}
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="department_initial">Department Initial</label>
         <input class="form-control border-input" placeholder="Department Initial" v-model="forms.department_initial"  type="text" maxlength="25" required>
                        <span class="label label-danger" v-for="error of errorNya['department_initial']">
                        {{ error }}
                    </span>
                    </div>
         ...
         <input class="form-control border-input" placeholder="Department Name" v-model="forms.department_name"  type="text" required>
                        <span class="label label-danger" v-for="error of errorNya['department_name']">
                        {{ error }}
                    </span>
                    </div>
...
<!-- @view --->
        ...
        <template slot="header" ><h4>View Company Department</h4></template>
        <template slot="body" >
                <div class="modal-body">

                    <!-- form Group -->
                    <div class="form-group">
                        <label for="company_name">Company Name :  {{forms.company_name}}</label>
                      
                    </div>

                    <!-- form Group -->
                    <div class="form-group">
                        <label for="department_name">Department Name :  {{forms.department_name}}</label>
                        
                    </div>
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="department_initial">Department Initial :  {{forms.department_initial}}</label>
...
<v-select v-model="forms.company_name" :value.sync="forms.company_name" :options="pilihannya" label="company_name"></v-select>
                        <span class="label label-danger" v-for="error of errorNya['company_id']">
                        {{ error }}
                    </span> 
          <span class="label label-danger" v-for="error of errorNya['company_id']">
                    ...
                    <div class="form-group">
                        <label for="department_initial">Department Initial</label>
         <input class="form-control border-input" placeholder="Department Initial" v-model="forms.department_initial"  type="text" maxlength="25">
                        <span class="label label-danger" v-for="error of errorNya['department_initial']">
                        {{ error }}
                    </span>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="department_name">Department Name</label>
         <input class="form-control border-input" placeholder="Department Name" v-model="forms.department_name"  type="text">
                        <span class="label label-danger" v-for="error of errorNya['department_name']">
                        {{ error }}
                    </span>
                    </div>
...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <tr>
      ...
      <td colspan="3"><input type="text" v-model="filterText" class="form-control" @keyup.enter="doFilter" placeholder="Department Name / Initial"></td>
    </tr>
     <tr>
    ...
    <vuetable ref="vuetable"
      api-url="/kevler/kevlerDepartment"
      :fields="fields"
      pagination-path=""
        track-by="id"
      :per-page="perPage"
...
export default {
  ...
  data () {
    ...
    formErrors:{},
    errors: new Errors() ,
    forms:new CrudForm({index:'',  id:'' , company_id:'' ,  department_initial:'' , department_name:'' , company_name:'' , created_at:''}) ,
    ...
    selectedTo: [],
      fields: [
        ...
        {
          name: 'department_initial',
		  title: 'Department Initial',
		  titleClass: 'text-center',
          dataClass: 'text-center'
        },
        {
  ...
  methods: {
   ...
   var check = confirm("Are you sure you want to delete this row?");  
                ...
                //console.log(join_selected_values);
                    ...
                    axios.delete('/kevler/kevlerDepartmentDeleteAll/'+ this.join_selected_values)
                    .then(response => {
                this.errorNya = '';
      ...
      this.loading2 = true;
                axios.get('/kevler/kevlerDepartmentGet?token=' + this.token).then((response) => {
  ...
  fetchIt(){
                ...
                this.loading = true;
                axios.get('/kevler/GetUserAccess/sub/cp2').then((response) => {
                    this.loading = false
                }).catch(error => {
  ...
  resetOptions() {
    ...
    submitBooks() {
	if(!this.forms.company_id)
	{ 
	
		this.errorNya = {company_id:['Company field is required']};
	}
	else
	{
				var data = {company_id:this.forms.company_id['id'],department_initial:this.forms.department_initial,department_name:this.forms.department_name};

            axios.post('/kevler/kevlerDepartment', data)
                    .then(response => {
					                    if(response.data.success)
{
			...
			if(response.data.department_initial)
{
  this.errorNya = {department_initial:[response.data.department_initial]};
}
			if(response.data.department_name)
{
  this.errorNya = {department_name:[response.data.department_name]};
}		
					})
                    .catch(error => {
                        this.errorNya = error.response.data;
   ...
   console.log(this.forms.company_name);
   /*
            axios.put('/kevler/kevlerDepartment/'+ this.forms.id , this.forms)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
            ...
            destroyItem(kode){
                axios.delete('/kevler/kevlerDepartment/'+ kode)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/kevler/kevlerDepartment').then((response) => {
                    this.loading = false
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
