...
<!-- for header  -->            
  ...
  <table class="table">
      ...
      <tr v-if="PesanData.status_id > 0">
        <td><h5 align="left"><b>Pesan</b></h5></td>
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectProcurementReviewer"
      :fields="fields"
      pagination-path=""
...
var masuk ={projectid:this.PesanData.id,tenantid:this.PesanData.tenantscode,message:this.message};
              ...
              axios.post('/karyawan/ProcurementApproval', masuk)
                    .then(response => {
if(response.data.success)
{
                  this.errorNya = '';
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetProjectProcurementReviewer').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
