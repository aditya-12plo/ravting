<template>
 ...
 <vue-toast ref='toast'></vue-toast>
      <h1 align="center">
       List Approval Project Tower UNTAPPED
      </h1>
    </section>
...
{{formatNumberRupiah(this.totalBiayaStandardB2S())}}
      ...
      <tr>
        ...
        <td align="right" colspan="2"><h4><b>{{formatNumberRupiah(TotalCapexB2S())}}</b></h4></td>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" @click="modal.set('reject', true)">Perbaiki Ke {{this.levelnya}}</button>
					<button type="button" class="btn btn-warning" @click="modal.set('direktur', true)">Persetujuan Direktur</button>
	...
	<!-- @simulasiB2S -->
        <modal  v-if="modal.get('simulasiB2S')" @close="modal.set('simulasiB2S', false)" style="width: 100%;margin: 0px auto;padding: 10px 20px;background-color: none;border-radius: 2px;box-shadow: 0 2px 8px rgba(0, 0, 0, .33);transition: all .3s ease; font-family: Helvetica, Arial, sans-serif;">
        <template slot="header" ><h4 align="center">Simulasi CAPEX APPROVAL</h4></template>
        <template slot="body" >
<form method="POST" action="" @submit.prevent="SubmitDataMasal()">		
  <div class="modal-body">
...
<div class="col-sm-12">		
	...
	<div class="col-sm-4">				
<div class="small">
<p><h5 align="center"><b>CAPEX NASIONAL ({{this.masalNasional.total}} SITE)</b></h5></p>
    <CapexNasional :height="300" :data="this.masalNasional"></CapexNasional>
  </div>
</div>

	<div class="col-sm-8">				
<div class="small">
<p><h5 align="center"><b>CAPEX AREA</b></h5></p>
    <CapexArea :height="300" :data="this.masalArea"></CapexArea>
  </div>
<div class="table-responsive">
  <table class="table">
    <tr>
   <td><h5 align="center"><b>AREA</b></h5></td>
   <td><h5 align="center"><b>BUDGET</b></h5></td>
   <td><h5 align="center"><b>CAPEX</b></h5></td>
   <td><h5 align="center"><b>TOTAL SITE</b></h5></td>
   </tr>

   <tr v-for="(row,index) in this.masalAreaList">
   <td><h5 align="center"> {{row.area}}</h5></td>
   <td><h5 align="center"> {{formatNumberRupiah(row.budgetNya)}}</h5></td>
   <td><h5 align="center"> {{formatNumberRupiah(row.capexNya)}}</h5></td>
   <td><h5 align="center"> {{row.total}}</h5></td>
   </tr>
		...
		<div class="col-sm-12">				
<div class="small">
<p><h5 align="center"><b>CAPEX REGIONAL</b></h5></p>
    <CapexRegional :height="350" :data="this.masalRegional"></CapexRegional>
  </div>
  <div class="table-responsive">
  <table class="table">
    <tr>
   <td><h5 align="center"><b>REGIONAL</b></h5></td>
   <td><h5 align="center"><b>BUDGET</b></h5></td>
   <td><h5 align="center"><b>CAPEX</b></h5></td>
   <td><h5 align="center"><b>TOTAL SITE</b></h5></td>
   </tr>

   <tr v-for="(rowd,index) in this.masalRegionalList">
   <td><h5 align="center"> {{rowd.regional}}</h5></td>
   <td><h5 align="center"> {{formatNumberRupiah(rowd.budgetNya)}}</h5></td>
   <td><h5 align="center"> {{formatNumberRupiah(rowd.capexNya)}}</h5></td>
   <td><h5 align="center"> {{rowd.total}}</h5></td>
   </tr>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" @click="modal.set('simulasiB2S', false)" >Close</button>
...
<!-- @approvemasal -->
        <modal  v-if="modal.get('approvemasal')" @close="modal.set('approvemasal', false)">
        <template slot="header" ><h4 align="center">Kirim Pesan Project</h4></template>
        <template slot="body" >

            <form method="POST" action="" @submit.prevent="ApproveItemMasal()">
                <div class="modal-body">
  ...
  <textarea v-model="message" class="form-control" rows="5" id="message" placeholder="Pesan" required></textarea>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" @click="modal.set('approvemasal', false)" >Close</button>
                    <button type="submit" class="btn btn-primary">Kirim</button>
                </div>
            </form>
        </template>
  ...
  <table>
    ...
    <tr>
      <td colspan="4"><button class="btn btn-success" @click.prevent="doFilter">Go</button>
		  ...
		  <button type="button" class="btn btn-danger"  @click="sumSelectedItems()">Submit Ke PROCUREMENT</button>
		  </td>
    </tr>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectApprovalEGMUUNTAPPED"
      :fields="fields"
      pagination-path=""
  ...
  props: ["data"],
    data () {
      return {
        ...
        datasets: [
		{
          label: 'PLAFON ('+this.formatGrafikNumberRupiah(this.data.budgetNya)+')',
          borderColor: '#05CBE1',
          pointBackgroundColor: 'white',
          pointBorderColor: 'white',
          borderWidth: 1,
          backgroundColor: '#dd4b39',
          data: [this.data.budgetNya]
        },
		{
          label: 'CAPEX ('+this.formatGrafikNumberRupiah(this.data.capexNya)+' )',
          borderColor: '#05CBE1',
          pointBackgroundColor: 'white',
          pointBorderColor: 'white',
          borderWidth: 1,
          backgroundColor: '#1451A1',
          data: [this.data.capexNya]
        }
        ]
        },
...
export default {
  ...
  option: {
    ...
    modal:new CrudModal({approvemasal:false,simulasiB2S:false,view:false,simulasi:false, reject:false, approve:false,  direktur:false,  repair:false, message:false}),
     berhasilNya: [],
	 simulasiNya: [],
simulasiNyaKedua: [],
     errorNya: [],
...
this.masalAreaList='';
this.masalRegional='';
this.masalRegionalList='';
this.masalKode='';	
  this.isLoading = true;
   var ttl = this.$refs.vuetable.selectedTo;
   if(ttl.length <= 0)
   {
   alert ('Pilih Data Yang Ingin Di Kirim');
   }
   else
   {
                var join_selected_values = ttl.join(","); 
                //console.log(ttl);  			
var masuk = {data:ttl,infratype:'UNTAPPED'};		
   axios.post('/karyawan/ApprovalEGMUmasal', masuk)
                    .then(response => {
this.errorNya = '';
this.message = '';
...
this.masalNasional=response.data.nasional;
this.masalArea= {'area':response.data.area,'budgetNya':response.data.listareabudgetNya,'capexNya':response.data.listareacapexNya,'total':response.data.listareatotal};
this.masalAreaList= response.data.sa;
this.masalRegional={'regional':response.data.regional,'budgetNya':response.data.listregionalbudgetNya,'capexNya':response.data.listregionalcapexNya,'total':response.data.listregionaltotal};
this.masalRegionalList=response.data.sr;
this.masalKode=response.data.kode;
this.isLoading = false;
this.modal.set('simulasiB2S', true); 
                    });  
...
fetchIt(){
this.isLoading = true;
                axios.get('/karyawan/GetProfileUbisReviewerEGMU').then((response) => {
...
var masuk ={id:this.masalKode,
 infratype:'UNTAPPED',
 message:this.message,
 levelnya:'STAFF PROCUREMENT',
 statusnya:'PROCUREMENT - STAFF PROCUREMENT'};
 
 
   axios.post('/karyawan/ApprovalProjectOfficeMasal', masuk)
                    .then(response => {
...
this.masalKode='';	
                this.resetFilter();
                 this.modal.set('approve', false);
                 this.modal.set('approvemasal', false);
				  this.modal.set('simulasi', false);
                 this.modal.set('view', false);
				 this.modal.set('simulasiB2S', false); 
				  this.isLoading = false;
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            SubmitDataMasal(){
                   ...
                   this.errorNya = '';
				   this.message = '';
                this.modal.set('approvemasal', true); 
...
onLoading() {
    ...
    onLoadingError() {
                this.isLoading = true;
                axios.get('/karyawan/GetProjectApprovalEGMUUNTAPPED').then((response) => {
                    this.isLoading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
