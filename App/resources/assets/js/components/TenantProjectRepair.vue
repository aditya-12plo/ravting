...
<!-- @view --->
		...
		<form method="POST"  action="" @submit.prevent="updateItem()" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="regional">Daerah : {{forms.header.regional}}</label>
                    </div>
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="tower">Reject Oleh {{forms.pesan.name}} :  {{forms.pesan.message}}</label>
                    </div>
					
					
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="tower">Dokumen Upload : <input type="file" name="file_name" id="file_name" v-on:change="newAvatar"> 
		<br>
		<button type="submit" class="btn btn-primary" @click.prevent="uploadItem()">Upload File</button> <font v-if="forms.header.filename"><a v-bind:href="url+forms.header.filename" target="_blank"><button type="button" class="btn btn-success">Download</button></a> <button type="submit" class="btn btn-danger" @click.prevent="DeleteFile()">Hapus File</button></font> 
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="message">Pesan Perbaikan Dari {{forms.pesan.name +' - '+ forms.pesan.level}} : {{forms.pesan.message}}</label></td>                   
				   </div>
...
<tr>
	  ...
	  <tr>
        <td align="center" colspan="4">Total Biaya Implementasi Standar (A)</td>
...
<tr>
	  ...
	  <tr v-for="(row,index) in this.additional">
	...
	<tr>
        ...
        <td colspan="1"><b>{{formatNumberRupiah(formatNumber(total(this.additional)))}}</b></td>
      </tr>
	  <tr>
        <td colspan="6"> <div style="height: 50px; overflow:hidden;"></div></td>
      </tr>
	...
	<tr>
        <td align="center" colspan="6"><h4 align="center"><b>PENGURANGAN BIAYA IMPLEMENTASI PEKERJAAN</b></h4></td>
      </tr>
	  ...
	  <tr v-for="(row,index) in this.reduction">
        <td><input type="text" v-model="row.workitem" class="form-control" required></td>
      <td><input type="text" v-model="row.unit"  maxlength="10" class="form-control" required></td>
      <td><input v-on:keypress="isNumber()"  maxlength="4" v-model="row.volume" class="form-control" required></td>
      <td><input v-on:keypress="isNumber()"  maxlength="13" v-model="row.unitprice" class="form-control" required></td>
    ...
    <tr>
        ...
        <td colspan="1"><b>{{formatNumberRupiah(formatNumber(total(this.reduction)))}}</b></td>
      </tr>
	  <tr>
        <td colspan="6"> <div style="height: 50px; overflow:hidden;"></div></td>
      </tr>
    <tr>
        ...
        <td colspan="1"><b>{{formatNumberRupiah(formatNumber((total(this.standart) + total(this.additional)) - total(this.reduction)))}}</b></td>
      </tr>
  </table>
  </div>
  </div>
        ...
        <template slot="body" >
                <div class="modal-body">
                    ...
                    <div class="form-group" v-for="(row,index) in this.pesannya">
                        <label for="pesan">Pengirim : {{row.name}}</label><br>
                        <label for="pesan">Pesan : {{row.message}}</label><br>
                        <label for="pesan">Waktu : {{row.created_at}}</label><br>
                    </div>
...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <tr>
      ...
      <td colspan="3"><input type="text" v-model="filterText" class="form-control" @keyup.enter="doFilter" placeholder="Site Name"></td>
    </tr>
     <tr>
    ...
    <vuetable ref="vuetable"
     api-url="/tenant/GetProjectRepair"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
Vue.component('project-button', require('./ProjectCustomActions.vue'))
Vue.use(VueEvents)
...
export default {
  ...
  data () {
	...
	url:'/files/',
    errors: new Errors() ,
    forms:new CrudForm({index:'',  id:'' , header:'' ,  pesan:[]}) ,
    ...
    loading: false,
      ...
      titleClass: 'text-center',
        ...
        {
          name: '__component:project-button',
          title: 'Actions',
          titleClass: 'text-center',
          dataClass: 'text-center'
        }
  ...
  methods: {
   addRow: function(){
       this.additional.push({workitem:"",unit:"",volume:"",unitprice:""});
    },
   addRowMin: function(){
       this.reduction.push({workitem:"",unit:"",volume:"",unitprice:""});
    },
    removeRow: function(row){
    //console.log(row);
    this.additional.splice(row,1)
    },
    removeRowMin: function(row){
    //console.log(row);
    this.reduction.splice(row,1)
    },
     newAvatar(event) {
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
			...
			axios.get('/tenant/GetRepairProjectDetail/'+item.id).then((response) => {
                this.forms.setFillItem(response.data , index );
...
if(response.data.success)
{
				  ...
				  axios.get('/tenant/GetRepairProjectDetail/'+ this.forms.header.id).then((response) => {
                    this.errorNya = '';
                  this.file_name = '';
...
if(response.data.success)
{
  ...
  symbol: " M",precision: 0,format: "%v %s"})
   ...
   CheckMessage() {
			axios.get('/tenant/GetMessageNya/'+this.forms.header.id).then((response) => {
   ...
   updateItem() {
 var masuk ={projectid:this.forms.header.id,standart:this.standart,addtional:this.additional,reduction:this.reduction};
console.log(masuk);
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/tenant/GetProjectRepair').then((response) => {
