...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectUbisApproval1"
      :fields="fields"
      pagination-path=""
...
fetchIt(){
                axios.get('/karyawan/GetProfileUbisGM?token=' + this.token).then((response) => {
...
{	
  ...
  ApproveData(){
   ...
   var masuk ={projectid:this.PesanData.id,tenantid:this.PesanData.tenantscode,implementation:this.tambahan,message:this.message};
              ...
              axios.post('/karyawan/UbisApproval1', masuk)
                    .then(response => {
if(response.data.success)
{
                  this.errorNya = '';
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetProjectUbisApproval1').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
