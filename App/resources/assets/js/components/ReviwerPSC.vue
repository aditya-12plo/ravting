...
<!-- @view -->
        ...
        <template slot="header"><h4 align="center">NO PR {{forms.projectid}}</h4></template>
        <template slot="body" >
<form method="POST" action="" @submit.prevent="SubmitData()" enctype="multipart/form-data">
 ...
 <table class="table">
      ...
      <tr>
        <td align="left" colspan="3"><b><button type="button" class="button btn-primary" @click="addRow">+ Input No PR </button></b></td>
      </tr>
	   <tr>
<td> </td>
 <td align="left"> </td>
...
<div style="overflow-x:auto;">
  <table>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectPSC"
      :fields="fields"
      pagination-path=""
...
fetchIt(){
this.isLoading = true;
                axios.get('/karyawan/GetProfileOfficerPSC').then((response) => {
...
var masuk ={id:this.forms.id,
 ...
 pr:this.tambahan};
   ...
   axios.post('/karyawan/SubmitPR', masuk)
                    .then(response => {
...
onLoading() {
    ...
    onLoadingError() {
                this.isLoading = true;
                axios.get('/karyawan/GetProjectPSC').then((response) => {
                    this.isLoading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
