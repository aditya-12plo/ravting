...
<!-- @approve --->
        ...
        <template slot="header" ><h4 align="center">Kirim Pesan ke Divisi Berikutnya</h4></template>
        <template slot="body" >
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectAreaM"
      :fields="fields"
      pagination-path=""
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetProjectAreaM').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
