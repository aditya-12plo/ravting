...
<!-- for header  -->
  ...
  <table class="table">
      ...
      <tr>
        <td><h5 align="left"><b>Regional</b></h5></td>
      ...
      <tr>
        ...
        <td colspan="5"><font v-if="PesanData.filename"><a v-bind:href="url+PesanData.filename" target="_blank"><button type="button" class="btn btn-success">Download</button></a> </font></td>
      </tr>
      <tr>
        <td><h5 align="left"><b>Reject Project Oleh {{this.pesannya.name +' - '+ this.pesannya.level}}</b></h5>
		</td>
        <td colspan="5"><h5 align="left"><b>{{this.pesannya.message}}</b></h5></td>
      </tr>
	    <tr>
        <td colspan="6"> <div style="height: 50px; overflow:hidden;"></div></td>
      </tr>
 <tr>
        <td align="center" colspan="6"><h4 align="center"><b>BIAYA IMPLEMENTASI STANDART</b></h4></td>
      </tr>
      <tr>
...
<tr>
	  ...
	  <tr v-for="(row,index) in this.additional">
        ...
        <td>{{formatNumberRupiah(row.unitprice)}}</td>
        <td>{{formatNumberRupiah(formatNumber(row.unitprice * row.volume))}}</td>
		<td align="center"></td>
    </tr>
	<tr>
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/tenant/TGetProjectReject"
      :fields="fields"
      pagination-path=""
...
export default {
  ...
  return total.reduce(function(total, num){ return total + num }, 0);
   ...
   CheckMessage() {
			axios.get('/tenant/GetMessageNya/'+this.PesanData.header.id).then((response) => {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
             axios.get('/tenant/GetRejectProjectDetail/'+ item.id).then((response) => {

                   this.errorNya = '';
                   this.PesanData= response.data.header;
                   this.pesannya= response.data.pesan;
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/tenant/TGetProjectReject').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
