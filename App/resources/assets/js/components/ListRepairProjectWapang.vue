    ...
    <tr>
        ...
        <td><center><b>{{formatNumberRupiah(formatNumber((total(this.standart) + total(this.tambahan)) - total(this.pengurangan)))}}</b></center></td>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click.prevent="ApproveData()">Submit</button>
                </div>
                </form>
        </template>
        </modal>
...
<div style="overflow-x:auto;">
  ...
  <div v-show="loading"><i class="fa fa-spinner fa-spin"></i> Loading Data</div>
    <vuetable ref="vuetable"
      api-url="/karyawan/GetRepairProjectArea"
      :fields="fields"
      pagination-path=""
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/karyawan/GetRepairProjectArea').then((response) => {
                    this.loading = false
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
                    if (error.response.status = 500) {
