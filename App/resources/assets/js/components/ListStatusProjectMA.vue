...
<!-- @view-->
 ...
 <div class="col-sm-6">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="mitra">Mitra :  {{ forms.mitra }}</label>
                    </div>
 ...
 <table class="table">
	  ...
	  <tr>
        ...
        <td align="left" colspan="2"><b>Sewa Lahan (11 tahun include PPh)</b></td>
        <td align="center">LS</td>
        <td align="center">{{formatNumberRupiah(forms.qty_lahan)}}</td>
        <td align="center">{{formatNumberRupiah(forms.qty_lahan)}}</td>
 ...
 <table class="table">
	  ...
	  <tr>
        ...
        <td align="center">{{this.dataStandard.tonase}}</td>
        <td align="right">{{this.dataStandard.biayatower}}</td>
 ...
 <td align="left"> </td>
 <td align="left">5.{{index+1}}</td>
 <td align="left">{{roww.pekerjaan}}</td>
...
<td align="right">{{formatNumberRupiah(roww.qty * roww.harga)}}</td>
	 ...
	 <tr>
        <td align="center" colspan="6"><h4><b>Total Pekerjaan Standart :</b></h4></td>
        <td align="right" colspan="2"><h4><b>{{formatNumberRupiah(this.totalBiayaStandardB2S())}}
        </b></h4></td>
      </tr>
...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetProjectProgressMA"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
export default {
  ...
  data () {
    ...
    errors: new Errors() ,
	PesanData:[],
    forms:new CrudForm({index:'',  id:'' , no_doc:'' ,filename:'' ,  tinggitower:'' , proc:'' ,  ubis:'' , projectid:'' , sitename:'', siteid:'' , area:'', regional:'', cluster:'', kode_tower_depan:'' ,  tower:''  ,  kode_tower_belakang:'' , longitude:'' , latituted:'' , towerprovider:''  , infratype:'' , budget:'' ,  sis:'' ,  sitac:'' ,  imb:'' ,   imb_jasa:'' , qty_lahan:'', lahan:'' ,  biayatower:'' ,  cme:'' ,   pln:'' ,   addtransport:'' ,  addpln:'' ,  addcme:'' , provinsi:''  , kabupaten:''  , mitra:'' , created_at:''}) ,
  ...
  return totalBiayaStandardUNTAPPED;
   ...
   addRow: function(){
      this.tambahan.push({workitem:"",unit:"",volume:"",unitprice:"",description2:""});
    },
  ...
  this.isLoading = true;
                axios.get('/karyawan/GetProfileAreaMAG').then((response) => {
...
onLoading() {
    ...
    onLoadingError() {
                this.isLoading = true;
                axios.get('/karyawan/GetProjectProgressMA').then((response) => {
                    this.dataNya = response.data;
                    this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
