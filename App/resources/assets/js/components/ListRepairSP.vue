      ...
      <tr>
        ...
        <td align="right" colspan="2"><h4><b>{{formatNumberRupiah(TotalCapexB2S())}}</b></h4></td>
                ...
                <div class="modal-footer">
					...
					<button type="button" class="btn btn-warning" @click="modal.set('repairArea', true)">Perbaiki Ke AREA</button>
					<button type="button" class="btn btn-danger" @click="modal.set('reject', true)">Perbaiki Ke UBIS</button>                   
...
<!-- @repairArea -->
        <modal  v-if="modal.get('repairArea')" @close="modal.set('repairArea', false)">
        <template slot="header" ><h4 align="center">Pesan Project Harus Approval Ulang</h4></template>
        <template slot="body" >

            <form method="POST" action="" @submit.prevent="RepairItemArea()">
                <div class="modal-body">
...
<div class="col-sm-12" v-else>
 <!-- form Group -->
  ...
  <textarea v-model="message" class="form-control" rows="5" id="message" placeholder="Pesan" required></textarea>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" @click="modal.set('repairArea', false)" >Close</button>
                    <button type="submit" class="btn btn-primary">Kirim</button>
                </div>
            </form>
        </template>
...
export default {
  ...
  option: {
    ...
    modal:new CrudModal({view:false,simulasi:false, reject:false, approve:false,  repairArea:false,repair:false, message:false}),
     berhasilNya: [],
     errorNya: [],
     workitem:[],
     url:'/files/',
...
var masuk = {palanya:this.forms,standard:this.dataStandard,additional:this.tambahan,pesan:this.approved};		
 ...
 levelnya:'MANAGER PROCUREMENT',
 statusnya:'PROCUREMENT - MANAGER PROCUREMENT'};
...
if(response.data.success)
{
			  ...
			  RepairItemArea() {
var masuk ={id:this.forms.id,message:this.message,level:'MANAGER AREA',statusnya:'REPAIR AREA - AREA'};
              axios.post('/karyawan/RepairItemProject', masuk)
                    .then(response => {
if(response.data.success)
{
                ...
                this.resetFilter();
                 this.modal.set('reject', false);
                 this.modal.set('repairArea', false);
				 this.modal.set('view', false);
                 this.submitted = this.showTime('success',response.data.success);
}
                    })
